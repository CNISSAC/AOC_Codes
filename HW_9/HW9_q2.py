...
class Problem:
  ...
  def __init__(self):
    ...
    # self.pbs = np.array([[-2, 2, 1, 1,-1], [2, 2, 3, 1,2]])  # beacon positions
    ...
    # noise models
    self.Q = dt ** 2 * np.diag([.01, .01, .01, .0001])

  def b_h(self, x):

    p = x[:2]
    y = np.zeros(self.nb)
    H = np.zeros((self.nb, self.n))
  ...
  def br_h(self, x):
    ...
    p = x[:2]
    y = np.zeros(self.nb * 2)  # dim: (nb*2,), rank 1
    H = np.zeros((self.nb * 2, self.n))
  ...
  def uni_f(self, x, u):
    ...
    s = np.sin(x[2])
    r = self.radius
    x = x + np.array([c * r * u[0], s * r * u[0], u[1], 0])
    x = self.fix_state(x)
    A = np.array([[1, 0, -s * r * u[0], c * u[0]], [0, 1, c * r * u[0], s * u[0]], [0, 0, 1, 0], [0, 0, 0, 1]])
    print(x)
    return x, A
...
xt = np.array([0, 0, 0, prob.radius])  # true state
print(prob.radius)

P = np.diag([.01, .01, .01, .04])  # covariance
...
ax2[1].plot(ts, np.reshape(np.sqrt(Ps[3, 3, :]), Ps.shape[2]), label="sigma_r")
...
ax2[2].plot(xs[3, :], label='r_estimated')
ax2[2].set_xlabel('r')
ax2[2].set_ylabel('meters or radians')
