...
def fun(TF,update_rate,control_rate):  # update_rate is the rate of measurment like 5Hz, 10Hz, 20Hz
    ...
    for k in range(0, k_range):
        ...
        for i in range(len(predicts)):
            ...
            os_p_m[i, :, :] = np.linspace(measures[keys[i]],predicts[keys[i]],steps_left)
        ...
        # plt.grid()
        # plt.plot(os_p_m[1,:,0],os_p_m[1,:,1])
        # circle=plt.Circle(os_p_m[1, np.size(os_p_m,axis=1)-1, :], os_r_move[1], color='r', linewidth=2,
        #            fill=False)
    ...
    for k in range(np.size(xs,1)):  # plot  agent's trajectory and the position of moving obstacles
        ...
        if k%n == 0 and i_ob < np.size(os_p_move,1):
            for j in range(len(os_r_move)-1):
                circle_true = plt.Circle(os_p_move[j,i_ob,:], os_r_move[j]-0.5, color='r', linewidth=2,
                              fill=False)
